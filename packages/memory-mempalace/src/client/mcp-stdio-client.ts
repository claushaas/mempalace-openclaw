...
import type { ResolvedMemoryMempalacePluginConfig } from '../config.js';
import { McpProtocolError, McpToolNotFoundError } from '../errors.js';
...
function coerceStructuredPayload(result: unknown): unknown {
	...
	if (
		...
		const content = (result as McpToolCallResponse).content ?? [];
		const text = content
			.map((item) => item.text ?? '')
			.join('\n')
			.trim();
...
class StdioMcpConnection {
	...
	public constructor(config: ResolvedMemoryMempalacePluginConfig) {
	...
	public async close(): Promise<void> {
		this.rejectPending(new McpProtocolError('MCP connection closed.'));
		if (this.child) {
			this.child.kill();
		}
		this.child = null;
	...
	private async start(protocolVersion: string): Promise<void> {
		if (!this.command) {
			throw new McpProtocolError('MCP stdio command is not configured.');
		}
		...
		if (!initializeResult || typeof initializeResult !== 'object') {
			throw new McpProtocolError('MCP initialize returned an invalid result.');
		}
	...
	private handleStdoutChunk(chunk: string): void {
		...
		while (true) {
			...
			if (isFailureMessage(message)) {
				pending.reject(
					new McpProtocolError(message.error.message, {
						cause: message.error,
					}),
				);
				continue;
	...
	private writeMessage(message: JsonRpcMessage | JsonRpcRequest): void {
		if (!this.child) {
			throw new McpProtocolError('MCP process is not running.');
		}
...
function normalizeSearchResults(payload: unknown): MemorySearchResult[] {
	...
	return payload.map((item) =>
		parseWithSchema(
			MemorySearchResultSchema,
			item,
			'Invalid search result returned by MCP backend.',
		),
	);
}

function normalizeArtifact(payload: unknown): MemoryArtifact {
	const candidate =
		payload &&
	...
	return parseWithSchema(
		MemoryArtifactSchema,
		candidate,
		'Invalid artifact returned by MCP backend.',
	);
}

function normalizeStatusPayload(payload: unknown): NormalizedStatusPayload {
	if (!payload || typeof payload !== 'object') {
		throw new McpProtocolError(
			'Invalid status payload returned by MCP backend.',
		);
	}
	...
	const health = parseWithSchema(
		...
		'Invalid runtime health returned by MCP backend.',
	);
	...
	const sources = Array.isArray(record.sources)
		? record.sources.map((source) =>
				parseWithSchema(
					SourceStatusSchema,
					source,
					'Invalid source status returned by MCP backend.',
				),
			)
		: [];
...
export class McpStdioMemPalaceClient implements MemPalaceClient {
	...
	public constructor(
		private readonly config: ResolvedMemoryMempalacePluginConfig,
	) {
	...
	public async get(artifactId: string): Promise<MemoryArtifact> {
		try {
			...
			if (tools.has('mempalace_get_drawer')) {
				const result = await this.callTool('mempalace_get_drawer', {
					artifactId,
				});
				return normalizeArtifact(result);
			}

			const searchPayload = await this.callTool('mempalace_search', {
				limit: 1,
				query: artifactId,
			});
	...
	public async getHealth(): Promise<RuntimeHealth> {
		try {
			const statusPayload = normalizeStatusPayload(
				await this.callTool('mempalace_status', {}),
			);
			return statusPayload.health;
		} catch (error) {
	...
	public async promote(input: MemoryPromoteInput): Promise<MemoryArtifact> {
		try {
			const result = await this.callTool('mempalace_add_drawer', input);
			return normalizeArtifact(result);
		} catch (error) {
